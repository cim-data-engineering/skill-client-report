...
| Call                        | Window   | Feeds                                              |
| --------------------------- | -------- | -------------------------------------------------- |
| `aggregate_entity:"level"`  | 4 months | the snapshot grid                                  |
| `aggregate_entity:"site"`   | 7 months | the snapshot closing row (last 4) and the trend    |

Windows run from the first of the month to tomorrow, since `local_end_date` is exclusive and the current month is reported to date. Level rows are levels x months, so page when levels x 4 exceeds 80. Never use `aggregate_entity:"zone"` for the snapshot — that is one row per zone per month, hundreds of rows for the same picture. Drill to zone only when investigating a named level.
...
Same movement definition as the snapshot Chg column — current month to date minus the start month, in pp — so this figure equals the site row Chg in the snapshot below it. Name both endpoint values and their months. Both endpoints come from the `site` series already fetched. Score to 1dp, matching the snapshot.
...
Heatmap table on the same `heatmap` component as the equipment health snapshot. One row per site level, four score columns, cell value the thermal comfort score to 1dp, closing with a signed change column.
